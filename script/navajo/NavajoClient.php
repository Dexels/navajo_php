...
class NavajoClient {
    ...
    static function processNavajo($service, $navajo) {
        ...
        if (!is_null($cerror)) {
            trace('<h2>SERVER SIDE</h2> Condition error calling service: ' . $service);
        }
    ...
    static function updateNavajoFromPost() {
        ...
        foreach ($_REQUEST as $current_var => $value) {
            ...
            if ($property->getAttribute('type') == 'selection') {
                # set correct option selected
                $n->setSelectedByValue($property, $value);
            } else
                if ($property->getAttribute('type') == 'boolean') {
